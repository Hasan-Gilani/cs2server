...
export default function GlovesPage() {
  ...
  const [playerGloves, setPlayerGloves] = useState([])  // {weapon_team, weapon_defindex}[]
  const [playerSkins,  setPlayerSkins]  = useState([])  // skin rows from wp_player_skins
  const [selectedId,   setSelectedId]   = useState(null) // selected glove weapon.id
  ...
  useEffect(() => {
    Promise.all([api.getGlovesCatalog(), api.getProfile()])
      .then(([cat, profile]) => {
        setCatalog(cat)
        setPlayerGloves(profile.gloves)
        setPlayerSkins(profile.skins)
        const firstId = cat[0]?.weapon?.id ?? null
  ...
  // Deduplicated glove type list for the sidebar — one entry per glove model
  ...
  const gloveTypeSkins = useMemo(() =>
    catalog.filter(s => s.weapon?.id === selectedId),
  [catalog, selectedId])

  const gloveRarities = useMemo(() => extractRarities(catalog), [catalog])

  const visibleSkins = useMemo(() => {
    const q = search.toLowerCase()
    return gloveTypeSkins
  ...
  }, [gloveTypeSkins, search, rarityFilter])

  // Which glove defindex is equipped per team: { 2: 5032, 3: 5032 }
  const equippedGloveByTeam = useMemo(() => {
    const m = {}
    for (const g of playerGloves) m[g.weapon_team] = g.weapon_defindex
    return m
  }, [playerGloves])

  // Skin equipped map: `defindex_team` → skin row (same pattern as KnifePage/WeaponsPage)
  ...
  async function handleApply(data) {
    ...
    try {
      ...
      await Promise.all(teams.map(t =>
        api.saveGloves({ weapon_team: t, weapon_defindex: data.weapon_defindex })
      ))
      ...
      setPlayerGloves(prev => {
        const filtered = prev.filter(g => !teams.includes(g.weapon_team))
        return [...filtered, ...teams.map(t => ({ weapon_team: t, weapon_defindex: data.weapon_defindex }))]
      })
      push('Gloves equipped!')
    } catch (e) {
  ...
  async function handleRemove(data) {
    try {
      ...
      await api.deleteGloves({ weapon_team: 0 })
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)]">
      ...
      <div className="px-5 py-2.5 border-b border-accent/10 text-xs text-slate-400 flex items-center gap-2 shrink-0"
        ...
        Pick a glove type, then choose a skin. Applying a skin also equips that glove type.
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-56 shrink-0 border-r border-white/[0.04] overflow-y-auto"
          ...
          <p className="px-4 pt-5 pb-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-accent/60">Glove Type</p>
          {gloveTypes.map(g => {
            const defindex = g.defindex
            const equippedT  = equippedGloveByTeam[2] === defindex
            const equippedCT = equippedGloveByTeam[3] === defindex
            const isActive = selectedId === g.weaponId
            return (
              <button
                key={g.weaponId}
                onClick={() => { setSelectedId(g.weaponId); setSearch(''); setRarityFilter(new Set()) }}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.04] shrink-0"
            ...
            <h2 className="font-bold text-lg text-white">
              {gloveTypes.find(g => g.weaponId === selectedId)?.name ?? 'Gloves'}
            </h2>
